...
<div class="container">
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('hrd.performance.questions.update', $question) }}">
                @csrf
                @method('PUT')
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <textarea id="question_text" name="question_text" class="form-control @error('question_text') is-invalid @enderror" rows="3" required>{{ old('question_text', $question->question_text) }}</textarea>
                        @error('question_text')
                    ...
                    <div class="col-md-6">
                        <select id="evaluation_type" name="evaluation_type" class="form-control @error('evaluation_type') is-invalid @enderror" required>
                            ...
                            @foreach($evaluationTypes as $value => $label)
                                <option value="{{ $value }}" {{ old('evaluation_type', $question->evaluation_type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
