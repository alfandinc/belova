...
<body>
    ...
    @php
        $categories = $labCategories->toArray();
    ...
    @for ($i = 0; $i < $totalCategories; $i += $categoriesPerRow)
        <div class="section-row">
            @for ($j = $i; $j < min($i + $categoriesPerRow, $totalCategories); $j++)
                <div class="section">
                    <div class="section-title">{{ strtoupper($labCategories[$j]->nama) }}</div>
                    <div class="test-container">
                        @foreach($labCategories[$j]->labTests as $test)
                            <div class="test-item">
                                <span class="checkbox {{ in_array($test->id, $labRequests) ? 'checked' : '' }}"></span>
                                {{ $test->nama }}
                            </div>
                        @endforeach
                    </div>
