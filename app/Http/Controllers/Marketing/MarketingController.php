...
class MarketingController extends Controller
{
    ...
    public function pasienData(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Pasien::select('id', 'nama', 'identity_document', DB::raw('identity_number as nik'), 'tanggal_lahir', 'gender', 'agama', 'marital_status', 'pendidikan', 'pekerjaan', 'gol_darah', 'notes', 'alamat', 'no_hp', 'no_hp2', 'email', 'instagram', 'status_pasien', 'status_akses', 'status_review')
                ->addSelect([
    ...
    public function getRevenueData(Request $request)
    {
        try {
            ...
            $obatIsSales = trim((string) $request->input('obat_is_sales', ''));
            ...
            $data = [
                ...
                'topObatRevenue' => $this->getTopObatRevenueItems($year, $startDate, $endDate, $clinicId, $obatSearch, $obatCategory, $obatIsSales),
    ...
    private function getTopObatRevenueItems($year, $startDate = null, $endDate = null, $clinicId = null, $search = '', $category = '', $isSalesFilter = '')
    {
        $baseQuery = InvoiceItem::join('finance_invoices', 'finance_invoice_items.invoice_id', '=', 'finance_invoices.id')
            ->join('erm_visitations', 'finance_invoices.visitation_id', '=', 'erm_visitations.id')
            ->join('erm_pasiens', 'erm_visitations.pasien_id', '=', 'erm_pasiens.id')
        ...
        if ($isSalesFilter === '1') {
            $baseQuery->where('erm_pasiens.is_sales', 1);
        } elseif ($isSalesFilter === '0') {
